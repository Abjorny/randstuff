$(document).ready(function () {
  ...
  function wordCount($n, $words) {
    var $x = ($xx = $n % 100) % 10;
    return $words[
      ($xx > 10 && $xx < 15) || !$x || ($x > 4 && $x < 10) ? 2 : $x == 1 ? 0 : 1
    ];
  }
      ...
      success: function (data) {
        if (!data.error) {
          if (!data.question.restart) {
            ...
            container
              .find(".text i")
              .attr("class", "lvl-" + data.question.level);
      ...
      success: function (data) {
        if (!data.error) {
          if (true === data.answer.success) {
            ...
            elem.addClass("err");
            container
              .find('.answers [data-num="' + data.answer.correct + '"]')
              .addClass("suc");
            container.find(".answers .item").addClass("dis");
          }
      ...
      timer
        .text(
          (!days ? "" : days + " " + wordCount(days, daysWords) + " ") +
            (!days && !hours
              ? ""
              : hours + " " + wordCount(hours, hoursWords) + " ") +
            (!days && !hours && !minutes
              ? ""
  ...
  $("#delete-number .button, #delete-contest .button").click(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.ok) {
          profile
            .find('[data-type="' + type + '"] [data-hash="' + hash + '"]')
            .remove();
  ...
  $(".show-text-all").click(function () {
    $(this).closest(".text-wrap").hide().next(".text-all").show();
